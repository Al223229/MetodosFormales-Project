  ...
  return (
    <div className="SearchBar">
      <input
        ...
        type="search"
        placeholder="Buscar..."
        className="TextInSearchBar"
      />
    </div>
  );
}

// 1. Recibimos cartItems y la función para abrir el carrito
function Header({ cartItems, onCartClick }) {
  const [isDropdownOpen, setDropdownOpen] = useState(false);

  // 2. Calculamos el total de items (sumando las cantidades individuales)
  const totalItems = cartItems.reduce((acc, item) => acc + item.quantity, 0);

  const categories = [
    { name: "Proteínas", link: "/Productos?category=proteinas" },
    { name: "Vitaminas", link: "/Productos?category=vitaminas" },
    { name: "Creatinas", link: "/Productos?category=creatinas" },
    { name: "Aminoácidos", link: "/Productos?category=aminoacidos" },
  ];

  return (
    <>
      <header className="Header flex items-center gap-6 px-6 relative">
        {/* Logo */}
        <img src={logo} alt="Logo Befit" className="h-12 object-contain" />

        {/* Botón Productos */}
        <div className="flex items-center gap-2 absolute top-[25px] left-[230px]">
          <button
            type="button"
        ...
        {/* Navegación derecha */}
        <nav
          className="inline-flex items-center gap-10 absolute top-[22px] right-[40px]"
          aria-label="User navigation"
        >
          {/* Botón perfil */}
          <button type="button" aria-label="User profile">
            <img className="w-[35px] h-[35px]" alt="Profile" src={usuario} />
          </button>

          {/* Botón Pedidos (Orders) */}
          <button type="button" aria-label="Orders">
             {/* ... (Tu SVG de orders se mantiene igual) ... */}
            <svg width="45" height="45" viewBox="0 0 43 38" fill="none" xmlns="http://www.w3.org/2000/svg" className="relative">
          ...
          {/* Carrito Modificado */}
          {/* 3. Agregamos position relative al botón contenedor y el evento onClick */}
          <button 
            type="button" 
      ...
      {/* Dropdown animado fuera del header */}
      <div
        className={`transition-all duration-300 ease-in-out overflow-hidden bg-[#B8D2B1] shadow-inner ${
