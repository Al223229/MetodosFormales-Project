...
const ProductDetail = () => {
  ...
  const [selectedImage, setSelectedImage] = useState(0); // Estado para la imagen seleccionada

  useEffect(() => {
    window.scrollTo(0, 0);
    setSelectedImage(0); // Reinicia a la primera imagen al cambiar de producto
    setQuantity(1);
  }, [id]);
    ...
    >
      <div className="container mx-auto px-4 xl:px-32 max-w-screen-2xl">
        ...
        >
          ...
          {/* Galería - CONTENEDOR NUEVO PARA EL HOVER */}
          <motion.div
            className="w-full lg:w-1/2 flex gap-4 h-[450px]"
            onHoverEnd={() => setSelectedImage(0)} // Al sacar el mouse de la galería, vuelve a la primera imagen
          >
            <div className="flex flex-col gap-3 w-20 flex-shrink-0 overflow-y-auto no-scrollbar">
              {[...Array(4)].map((_, idx) => ( // Asumimos 4 imágenes de ejemplo
                <motion.button
                  key={idx}
                  onHoverStart={() => setSelectedImage(idx)} // Cambia la imagen al pasar el mouse
                  ...
                  whileHover={{ scale: 1.00 }} // Animación de escala al pasar el mouse
                >
                  <img
                    // Usamos una URL dinámica de placeholder con el índice
            ...
            <div className="flex-1 flex items-center justify-center bg-gray-50 rounded-xl p-4 overflow-hidden">
              <AnimatePresence mode="wait">
                <motion.img
                  key={selectedImage} // Clave para que AnimatePresence detecte el cambio
                  ...
                  initial={{ opacity: 0, scale: 0.95 }} // Empieza más pequeña y transparente
                  animate={{ opacity: 1, scale: 1 }} // Crece y se hace visible
                  exit={{ opacity: 0, scale: 0.95 }} // Se hace pequeña y transparente al salir
                  transition={{ duration: 0.2, ease: "easeInOut" }}
                />
              </AnimatePresence>
            </div>
          </motion.div>
          {/* FIN Galería - CONTENEDOR NUEVO */}
